...
const randomizeInk = (text: string) =>
  text.split('').map((char, idx) => (
    <span
      key={`${char}-${idx}`}
      style={{ opacity: Math.random() > 0.15 ? 1 : 0.4 }}
    ...
    >
      <div className="border-4 border-dashed border-black/70 p-4 space-y-3" style={{ background: 'linear-gradient(#fff 0%, #f7f7f7 100%)' }}>
        <div className="text-center">
          <p className="text-[10px] tracking-[0.28em]">{randomizeInk('RECEIPT')}</p>
          <h2 className="text-lg font-bold tracking-wide mt-1">{randomizeInk(title)}</h2>
        ...
        <div className="border-t border-b border-black/60 py-2 space-y-2 text-sm">
          {lines.map((line, idx) => (
            <div key={idx} className="flex justify-between">
              <span>{randomizeInk(line.label)}</span>
              <span className="font-semibold">{randomizeInk(line.value)}</span>
            </div>
          ))}
        </div>
        ...
        <div className="text-center space-y-1">
          ...
          <p className="text-xs tracking-widest font-bold mt-1">{randomizeInk(triggerPhrase)}</p>
        </div>
